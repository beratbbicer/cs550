        ...
        while True:   
            ...
            for i in range(self.distance_matrix.shape[0]):
                if self.distance_matrix[x, i] != alot:
                    d = distance.cdist(x_data, np.reshape(self.data[np.argwhere(self.assignments == i)], (-1,2)), "euclidean")
                    tmp = d[np.unravel_index(np.argmax(d, axis=None), d.shape)]
                    self.distance_matrix[x, i] = tmp # max(tmp, self.distance_matrix[x, i])
                    self.distance_matrix[i, x] = tmp # max(tmp, self.distance_matrix[i, x])
